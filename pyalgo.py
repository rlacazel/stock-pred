...
def fetch_data(stock_sym, override_file = False, from_date = '2015-01-01', to_date = '2018-01-01'):
...
# feed = yahoofeed.Feed()
# file = fetch_data('ALXN')
# feed.addBarsFromCSV("data", file)
	
# bb = strategies.BBands(feed, "data", [40]) # , True, True
# run(bb, 'data', True)

# ordered = collections.OrderedDict(sorted(bb._actions.items(), key=lambda t: t[0]))
...
def runAllStrat(strats, weights):
	...
	for key in list(chain(*[s._actions.keys() for s in strats])):
		orders[key] = dict()
		for strat in strats:
			if key in strat._actions:
				orders[key][strat.__class__.__name__] = strat._actions[key]
